...
class BigQmtRpcClient:
    ...
    def close(self):
        """Release the transport and any open connections.

        The swappable transport (ZMQ / MySQL / ...) runs background threads and
        holds a socket. Without an explicit stop those linger until the process
        exits, so a long-lived caller (e.g. a live trading scheduler) leaks a
        thread + socket per connection cycle. Calling this tears them down.
        Safe to call more than once and never raises.
        """
...
class BigQmtXtTrader:
    def __init__(
        ...
        self._event_channel = None  # active push channel, for prompt stop()
    ...
    def stop(self):
        self._event_running = False
        # Promptly unblock the event loop: if it is sitting in a push-channel
        # subscriber, stop that channel so the loop's flag check returns at once
        # instead of waiting out its 0.5s sleep.
        ch = getattr(self, "_event_channel", None)
        if ch is not None:
            try:
                ch.stop()
            except Exception:
        ...
        if thread is not None and thread.is_alive():
            thread.join(3.0)
        self._event_thread = None
        # Stop the async-order worker. It blocks on the queue, so a None
        # sentinel makes it return. Only matters if order_stock_async was used.
        aw = getattr(self, "_async_order_thread", None)
        if aw is not None and aw.is_alive():
            try:
                self._async_order_queue.put(None)
                aw.join(2.0)
            except Exception:
                pass
        self._async_order_thread = None
        # Tear down the RPC transport so its background threads (ZMQ router /
        # stall) and socket are released instead of lingering until process
        # exit. This is the fix for "QMT closed but threads still occupy" — a
        # long-running live service previously leaked one thread + socket per
        # run_live cycle because stop() only joined the event listener.
        try:
    ...
    def close(self):
        """Idiomatic alias for stop(); some callers prefer close()."""
        return self.stop()
